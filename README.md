...
> 
> 把 Hermes 变成一个**会自己纠错**的编程智能体：通过「实现→校验→修复→全绿才完成」的硬循环，让中档及以上模型在真实编程题上的产出可靠、可验证——机器门禁强制，不靠提示词自觉。
>
> **模型支持门槛**：编码特化模型 ≥8B 激活参数 / 通用模型 ≥24B 为正式支持档；更小的模型可实验性使用（门禁仍会拦截假完成与篡改测试），但不担保收敛。实测依据：任何档位的模型无 harness 时都会假报完成（含 118B 编码特化模型），而 <8B 模型会绕过门禁甚至篡改裁判文件。
