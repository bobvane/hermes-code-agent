...
1. **Delegate by default for non-trivial tasks.** Do not implement + review in the same head. Use `delegate_task`: one builder agent (isolated context) + one reviewer agent, run in parallel, then reconcile. Use the reusable prompts in `references/parallel-implement-review.md`. Matches OpenCode's Task-tool fan-out and Codex's `multi_agents_v2` (spawn → send_message → wait → interrupt).
