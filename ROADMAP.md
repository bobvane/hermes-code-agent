...
**v1.9.0** (2026-08-25): 第一批功能落地（七项开发目标全部实施）— ①计划/执行分离 v2：SKILL.md 双段模式指令文本(Codex ModeKind) + clarify 阶段门(Cline Plan/Act) + plancheck 必经点；②测试反馈重试循环 v2：三通道分流(补丁3/静态3/测试5 各自计数) + `locate` did-you-mean 模糊定位救援(difflib, Aider 移植)；③步数/花费封顶 v2：8k 提醒带进度保存指令(Codex context_window_reminder 口径) + MAX_STEPS 收尾协议(OpenCode, DONE/NOT DONE/NEXT) + 显式步骤上限；④子代理并行 v2：委派默认化触发清单(OpenCode) + 六动作生命周期编排协议(Codex spawn/steer/wait/list/stop/re-brief)；⑤每角色不同模型：角色→档次声明式映射表 + 非阻断边界提醒；⑥权限审批分级：L0/L1/L2/L3 四档操作分级表(clarify 为审批动作, Codex approval policy × Cline auto-approve)；⑦危险命令拦截：`check_cmd` 子命令 + `cmd_policy.yaml` 规则表——Codex execpolicy 三态前缀匹配(逐段裁决/最长前缀优先/fail-closed) × Gemini 引号感知命令替换扫描($()、反引号、<()/>() )。README 重写为安装方法+功能对照表。自测 56/56。
